...
use crate::util::{
    constants::{MINDLESS_DIR_NAME, NEVERMIND_FILE_NAME},
    output::print_project_not_found,
};
...
pub fn get_nevermind_patterns(mindless_root: &PathBuf) -> Vec<Pattern> {
    ...
    let default_pattern = Pattern::new(&sanitized_default_pattern)
        .expect("Something went wrong while creating default pattern");
    ...
    let error_msg = format!(
        "nevermind file at {} could not be read",
        nevermind_file_path.display()
    );
    let content = fs::read_to_string(nevermind_file_path).expect(&error_msg);

    for line in content.lines() {
        let sanitized_pattern = line.trim().trim_start_matches("/").trim_end_matches("/");
...
pub fn get_tracked_files(
    ...
    for child in children {
        ...
        let path_str = child_path
            .to_str()
            .expect("Something went wrong while reading file path");
        ...
        if child_path.is_dir() {
            let descendants =
                get_tracked_files(Some(&child_path), nevermind_patterns, &mindless_root);
